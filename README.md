...
字表以**注音符號**記錄讀音（如 `ㄓㄨㄥ`、`ㄬㄧㄛ˙`）。雙拼需要把音節拆成「聲母 + 韻母」兩段並各給一個鍵，因此先把注音轉寫成拉丁拼式——這套轉寫基於[老國音衆多拼音](https://github.com/baopaau/rime-bepemefe)方案，並自己做了調整：**以漢語拼音為基礎，凡與普通話讀音不同之處，拼寫就跟著不同**。所以看拼式就知道新老國音的差別。
