...
> 沿用小鶴雙拼的二十六個鍵，用**兩個鍵**打出老國音（《校改國音字典》）裏的任何一個音節。
...
字表以**注音符號**記錄讀音（如 `ㄓㄨㄥ`、`ㄬㄧㄛ˙`）。雙拼需要把音節拆成「聲母 + 韻母」兩段並各給一個鍵，因此先把注音轉寫成拉丁拼式——這套轉寫沿用「[國語拼音](https://github.com/baopaau/rime-bepemefe)」（`bepemefeve_pingyin`）方案：**以漢語拼音為基礎，凡與普通話讀音不同之處，拼寫就跟著不同**。所以看拼式就知道新老國音的差別。
...
**聲母**：`b p m f d t n l g k h j q x r z c s` 用本鍵；`zh ǝ→V`、`ch→I`、`sh→U`；三個老國音特有的聲母借鍵：**ㄫ(ng)→A**、**ㄬ(ny)→N**、**ㄪ(v)→V**（它們能配的韻母與同鍵的聲母不衝突）。
...
**③ 小鶴沒有、本方案新增的**：
- 三個借鍵聲母：**ㄫ→A**（我＝`ao`／岸＝`aj`）、**ㄬ→N**（你＝`ni`、年＝`nm`）、**ㄪ→V**（微＝`vw`）
...
- [bepemefeve 老國音注音字音表](https://github.com/baopaau/rime-bepemefe)（抱豹）——字表、注音碼表與「國語拼音」轉寫規則
